...
std::string SusiScanner::susiErrorToReadableError(const std::string& filePath, const std::string& susiError)
{
    ...
    else if (susiError == "recursion limit" || susiError == "unexpected (0x80040231)" || susiError == "archive bomb")
    {
        ...
        const std::string& userID)
{
    ...
    LOG_SUSI_INFO("Starting scan of " << file_path);
    SusiResult res = m_susi->scanFile(metaDataJson.c_str(), file_path.c_str(), fd, &scanResult);
    LOG_SUSI_INFO("Finished scanning " << file_path << " result: " << std::hex << res << std::dec);
