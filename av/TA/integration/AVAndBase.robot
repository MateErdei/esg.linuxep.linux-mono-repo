...
AV Plugin Restores Newest SafeStore Database Backup On Upgrade
    ...
    Check SulDownloader Log Contains    Successfully restored old SafeStore database (${AV_RESTORED_VAR_DIRECTORY}/2741684233_SafeStore_1.0.0) to ${SAFESTORE_DB_DIR}

Older SafeStore Database Is Not Restored When A SafeStore Database Is Already Present
    Register Cleanup  Exclude MCS Router is dead
    Register Cleanup  Install With Base SDDS
    Check AV Plugin Runnin
    ...
    Check SulDownloader Log Contains    SafeStore database already exists so will not attempt to restore: ${AV_RESTORED_VAR_DIRECTORY}/${safeStoreDatabaseBackupDirs[0]}
...
Older SafeStore Database Is Not Restored When It Is Not Compaitible With Current AV Version
    ...
    Check SulDownloader Log Contains    SafeStore Database (${newSafeStoreDatabasePath}) is not compatible with the current AV version (${version}) so will not attempt to restore
...
AV Plugin tries to restart threat detector on susi startup settings change
    ...
    AV Plugin Log Contains With Offset  Failed to connect to ${COMPONENT_ROOT_PATH}/chroot/var/process_control_socket - retrying after sleep
    ...
    AV Plugin Log Does Not Contain With Offset  Failed to connect to ${COMPONENT_ROOT_PATH}/chroot/var/process_control_socket - retrying after sleep
    Check Sophos Threat Detector has same PID   ${pid}
