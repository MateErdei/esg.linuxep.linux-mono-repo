...
def check_telemetry(telemetry):
    ...
    assert av_dict["lr-data-hash"] is not "unknown", "LR Data Hash is set to unknown in telemetry"
    assert av_dict["ml-lib-hash"] is not "unknown", "ML Lib Hash is set to unknown in telemetry"
    assert av_dict["ml-pe-model-version"] is not "unknown", "No ML-PE Model is set to unknown in telemetry"
    assert av_dict["vdl-ide-count"] is not "unknown", "No IDE Count is set to unknown in telemetry"
    assert av_dict["vdl-version"] is not "unknown", "No VDL Version is set to unknown in telemetry"
