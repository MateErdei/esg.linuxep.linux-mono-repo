...
def check_safestore_telemetry_dict(telemetry):
    ...
    assert "quarantine-successes" in safestore_dict, "No count of Quarantine Successes in telemetry"
    assert "quarantine-failures" in safestore_dict, "No count of Quarantine Failures in telemetry"
    assert "unlink-failures" in safestore_dict, "No count of Unlink Failures in telemetry"
    assert "database-deletions" in safestore_dict, "No count of Database Deletions in telemetry"
    assert "successful-file-restorations" in safestore_dict, "No count of Successful File Restorations in telemetry"
    assert "failed-file-restorations" in safestore_dict, "No count of Failed File Restorations in telemetry"
    ...
    assert safestore_dict["database-deletions"] == 0, "Count of SafeStore Database Deletions is not defaulting to 0"
    assert safestore_dict["successful-file-restorations"] == 0, "Count of SafeStore Successful File Restorations is not defaulting to 0"
    assert safestore_dict["failed-file-restorations"] == 0, "Count of SafeStore Failed File Restorations is not defaulting to 0"
