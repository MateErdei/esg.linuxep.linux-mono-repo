...
Threat Detector Log Contains With Offset For High Ml Scores
    [Arguments]  ${input}
    ...
    ${primary_score} =  Find Score  ${THREAT_DETECTOR_LOG_PATH}   "Primary score: "   offset=${offset}
    ${secondary_score} =  Find Score  ${THREAT_DETECTOR_LOG_PATH}   "Secondary score: "   offset=${offset}
    ${value} = Check High Scores  ${primary_score}  ${secondary_score}
    [return]  ${value}

Find Score
    [Arguments]  ${path}  ${find_word}  ${offset}=0
    Log To Console  Entered score
    Log To Console  Path: ${path}
    Log To Console  Offset: ${offset}
    ${value}   Set Variable  ""
    ${line}   Set Variable  ""
    Log To Console  Set variables
    ${content} =  Get File   ${path}  encoding_errors=replace
    Log To Console  Got content
#    change below back to just 'log'
    Log To Console   "Skipping ${offset} lines"
    @{lines} =  Split To Lines   ${content}  ${offset}
    Log To Console  Before FOR
    FOR  ${line}  IN  @{lines}
    Log To Console  Compare
        ${contents} =  Run Keyword And Return Status  Should Contain  ${line}  ${find_word}
        Log To Console  Before IF
        Exit For Loop If  ${contents} == True
           log to console   inside loop start
           @{string} =  Split String From Right  ${line}
           log to console  middle
           ${value} = @{string}[0]
           log to console   inside loop end
    END

    [return]  ${value}

Check High Ml Scores
    ${value}  Set Variable  ${0}
    [Arguments]  ${primary_score}  ${secondary_score}
    #When we get the new ML Lib, 16 should be corrected back to 20
    log to console   score comparison
    If  ${primary_score} > 30 && ${secondary_score} >=16
        ${value} = 1
    End
    [return]  ${value}
#
#Check Low Ml Score
#    ${value} = 0
#    [Arguments]  ${primary_score}
#    If  ${primary_score} < 30
#      ${value} = 1
#    End
#    [return]  ${value}

Threat Detector Log Contains For High Ml Scores
    ${value}  Set Variable  ${0}
    ${primary_score}   Set Variable  ""
#    ${secondary_score}   Set Variable  ""
    Log To Console  Entered
    [Arguments]  ${input}
...
#    File Log Contains With Offset  ${THREAT_DETECTOR_LOG_PATH}   ${input}   offset=${offset}
    ${primary_score} =  Find Score  ${THREAT_DETECTOR_LOG_PATH}  Primary score\:  offset=${offset}
    Log To Console  Primary score
#    ${secondary_score} =  Find Score  ${THREAT_DETECTOR_LOG_PATH}   "Secondary score: "   offset=${offset}
#    Log To Console  Secondary score
#    ${value} = Check High Ml Scores  ${primary_score}  ${secondary_score}
    Log To Console  Value
    [return]  ${value}
#
#Threat Detector Log For Low Ml Score
#    [Arguments]  ${input}
#    ${offset} =  Get Variable Value  ${SOPHOS_THREAT_DETECTOR_LOG_MARK}  0
#    File Log Contains With Offset  ${THREAT_DETECTOR_LOG_PATH}   ${input}   offset=${offset}
#    ${primary_score} =  Find Score  ${THREAT_DETECTOR_LOG_PATH}   "Primary score: "   offset=${offset}
#    ${value} = Check Low Ml Score  ${primary_score}
#    [return]  ${value}
