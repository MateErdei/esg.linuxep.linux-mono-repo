...
namespace
{
    class MockFanotifyHandler : public sophos_on_access_process::fanotifyhandler::IFanotifyHandler
    {
        ...
        MOCK_METHOD(int, markMount, (const std::string& path), (override));
        MOCK_METHOD(int, unmarkMount, (const std::string& path), (override));
        MOCK_METHOD(int, cacheFd, (const int& fd, const std::string& path), (override));
        MOCK_METHOD(int, clearCachedFiles, (), (override));
