...
TEST_F(TestPolicyProcessor, testProcessFlagSettingsEnabled)
{
    ...
    proc.processFlagSettings(R"sophos({"av.onaccess.enabled": true, "safestore.enabled": true, "safestore.quarantine-ml": true})sophos");
...
TEST_F(TestPolicyProcessor, testWriteFlagConfigFailedOnAccessDoesntPreventEnablingSafeStoreFlags)
{
    UsingMemoryAppender memAppend(*this);
