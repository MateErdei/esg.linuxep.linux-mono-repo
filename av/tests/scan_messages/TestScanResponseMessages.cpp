...
public:
    void SetUp() override
    {
        m_scanResponse.addDetection(m_filePath, m_threatType, m_threatName, m_sha256);
    }

    std::string m_filePath = "/tmp/eicar.com";
    std::string m_threatType = "virus";
...
TEST_F(TestScanResponseMessages, CreateScanResponse)
{
    ...
    EXPECT_EQ(detections[0].getThreatType(), m_threatType);
