...
namespace std
{
    ...
    inline istream& operator>>(istream& is, _Expect_str f)
    {
    ...
    inline istream& operator>>(istream& is, _Expect_char f)
    {
        char c;
        if (is.get(c) && (c != f.c))
            is.clear(ios::failbit);
    ...
    //
    inline istream& expect_eof(istream& is)
    {
        // GNU & MS have different semantics here, hence the odd multi-stage test.

        if (is.eof())
            return is;
        if (is.peek() == istream::traits_type::eof())
            is.clear(ios::eofbit);
        if (!is.eof())
            is.clear(ios::failbit);
        return is;
    ...
    inline istream& operator>>(istream& is, _Match f)
    {
        f.s = "";
        char c;
        int n = 0;
        while (is.get(c) && f.p(c) && ++n != f.max + 1)
        { // NOTE: this is not an off by one error
            f.s += c;
        }
        if (is)
            is.putback(c);
        if (n < f.min || n == f.max + 1)
            is.clear(ios::failbit);
        return is;
    ...
    inline istream& operator>>(istream& is, _Match_base64 f)
    {
        int lines;
    ...
    inline istream& operator>>(istream& is, _Get_upto f)
    {
        ...
        while (is.good())
        {
            ...
            string lookbehind = tmpstr.substr((tmpstr_len - f_delim_len), f_delim_len);
