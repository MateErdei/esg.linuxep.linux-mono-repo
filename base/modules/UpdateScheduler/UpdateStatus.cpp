...
namespace {
    ...
    std::string getsubscriptionsElement(const std::vector<UpdateScheduler::ProductStatus> & products)
    {
        ...
        for( auto & product: products)
        {
            std::string subscriptionLine = StringUtils::orderedStringReplace(subscriptionTemplate, {
                ...
                {"@@displayversion@@", product.InstalledVersion}
            });
...
std::string UpdateScheduler::SerializeUpdateStatus(const UpdateScheduler::UpdateStatus &status, const std::string &revID,
