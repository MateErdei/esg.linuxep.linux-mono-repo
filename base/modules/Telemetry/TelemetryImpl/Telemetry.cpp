...
namespace Telemetry
{
    int main_entry(int argc, char* argv[], std::shared_ptr<IHttpSender> httpSender)
    {
        try
        {
            if (argc != 2 && argc != 4)
            {
                throw std::runtime_error("Telemetry executable expects either 1 (verb) or 3 (verb, server, port) arguments");
            }
            ...
            if (verb == "POST")
            {
                ...
                httpSender->postRequest(additionalHeaders,
                                         jsonStruct); // [LINUXEP-6075] This will be done via a configuration file
            }
            else if (verb == "GET")
            {
                httpSender->getRequest(additionalHeaders); // [LINUXEP-6075] This will be done via a configuration file
            }
            else
            {
