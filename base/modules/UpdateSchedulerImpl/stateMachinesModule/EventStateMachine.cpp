...
namespace stateMachinesModule
{
    ...
    bool EventStateMachine::DiscardRecentDuplicate(int updateError, const std::chrono::system_clock::time_point& now)
        const
    {
        return (updateError == state_.lastError) &&
               (now - state_.lastTime < StateData::EventMachineState::throttlePeriod);
    }
    ...
    EventStateMachine::EventStateMachine(
        ...
        downloadStateMachine_{ downloadStateMachine }, installStateMachine_{ installStateMachine }, state_{ state }
    {
    }
