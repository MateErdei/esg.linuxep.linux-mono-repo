...
def setup_logging():
    ...
    log_file = PathManager.register_log()
...
def create_dirs():
    ...
    paths = [
        ...
        path_manager.lock_sophos(),
        path_manager.sophosspl_log_dir(),
        path_manager.sophos_etc_dir(),
        path_manager.policy_dir(),
        path_manager.event_dir(),
        path_manager.action_dir(),
...
def add_options_to_policy(relays, proxycredentials):
    ...
    policy_config_path = PathManager.mcs_policy_config()
...
def stop_mcs_router():
    ...
    if "SubState=dead" not in output:
        subprocess.call([PathManager.wdctl_bin_path(), "stop", "mcsrouter"])
...
def start_mcs_router():
    ...
    output = subprocess.check_output(
        ["systemctl", "show", "-p", "SubState", "sophos-spl"])
    if "SubState=dead" not in output:
        subprocess.call([PathManager.wdctl_bin_path(), "start", "mcsrouter"])
...
def restart_update_scheduler():
    ...
    if "SubState=dead" not in output:
        ...
        subprocess.call([PathManager.wdctl_bin_path(),
...
def inner_main(argv):
    ...
    top_config = utils_config.Config(PathManager.mcs_router_conf())
    debug = os.environ.get("MCS_DEBUG", None)
    if debug:
        top_config.set("LOGLEVEL", "DEBUG")
        top_config.save()

    config = utils_config.Config(PathManager.root_config(), top_config)
    ...
    if token is not None and url is not None:
        ...
        policy_config = utils_config.Config(
            PathManager.mcs_policy_config(), config)
        try:
        ...
        client_config = utils_config.Config(PathManager.sophosspl_config())
    ...
    if token is not None and url is not None:
        ...
        if ret != 0:
            ...
            # cleanup RMS files
            safe_delete(PathManager.sophos_config_file())

            # cleanup last reported update event
            safe_delete(PathManager.get_update_last_event_file())
