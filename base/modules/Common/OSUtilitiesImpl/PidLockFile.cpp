...
namespace Common
{
    namespace OSUtilitiesImpl
    {
        ...
        PidLockFile::PidLockFile(const std::string& pidfile) :
                m_fileDescriptor(-1), m_pidfile(pidfile)
        {
            ...
            if (written < 0 || static_cast<size_t>(written) != pidLen)
            {
                pidLockUtils()->close(localfd);
                std::stringstream errorStream;
                errorStream << "Failed to write pid to pidfile"<< m_pidfile <<" because "<<strerror(errno)<<"("<<errno<<")";
                throw std::system_error( errno, std::generic_category(), errorStream.str());
            }
...
{
    static Common::OSUtilitiesImpl::IPidLockFileUtilsPtr instance = Common::OSUtilitiesImpl::IPidLockFileUtilsPtr(new Common::OSUtilitiesImpl::PidLockFileUtils());
    return instance;
}
