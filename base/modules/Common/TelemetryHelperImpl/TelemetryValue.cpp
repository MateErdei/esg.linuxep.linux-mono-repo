...
{
    TelemetryValue::TelemetryValue() : m_valueType(ValueType::unset) {}

    TelemetryValue::TelemetryValue(const std::string& value) : m_value(value), m_valueType(ValueType::string_type) {}

    TelemetryValue::TelemetryValue(const bool value) : m_value(value), m_valueType(ValueType::boolean_type) {}

    TelemetryValue::TelemetryValue(const int value) : m_value(value), m_valueType(ValueType::integer_type) {}
    ...
    TelemetryValue::TelemetryValue(const char* value) : m_value(std::string(value)), m_valueType(ValueType::string_type)
    {
    }
    ...
    TelemetryValue::ValueType TelemetryValue::getValueType() const { return m_valueType; }
    ...
    void TelemetryValue::checkType(ValueType expectedType) const
    {
        if (m_valueType != expectedType)
        {
            ...
            msg << "Telemetry value does not contain the expected type. Expected: " << static_cast<int>(expectedType)
                << "Actual: " << static_cast<int>(m_valueType);
