...
Test Connection Retry Backoff Period Is Set Correctly
    ...
    #back off can be transient, with a 5s or 15s
    ${initialbackoff} =   Wait For Back Off And Set Initial Value    ${mcs_mark}
    ...
    Check Log Contains String N Times   ${MCSRouterLog}   MCS Router Log   waiting up to ${initialbackoff} after 1 failures   1
    ...
    ...   waiting up to ${initialbackoff} after 1 failures
    ...   waiting up to 30.000000s after 2 failures
    ...   waiting up to 60.000000s after 3 failures

    ${timestamp1} =  get_time_difference_between_two_log_lines_where_log_lines_are_in_order   waiting up to ${initialbackoff} after 1 failures   Trying connection directly to localhost:4443     ${MCSRouterLog}
