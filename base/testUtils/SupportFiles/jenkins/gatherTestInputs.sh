...
python3 -m venv "${VENV}" && source "${VENV}/bin/activate"
  ...
  tar xzf ${SYSTEMPRODUCT_TEST_INPUT}/sspl-plugin-anti-virus/tap_test_output.tar.gz -C ${SYSTEMPRODUCT_TEST_INPUT}/sspl-plugin-anti-virus-test-tools  || fail "Error: Failed to unpack AV test tools"
