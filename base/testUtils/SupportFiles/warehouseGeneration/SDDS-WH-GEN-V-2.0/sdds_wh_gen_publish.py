...
class DeliberateErrors(object):
    ...
    def getSHA384_errors(self, key, sha384):
        ...
        self.logger.statusMessage(f" Replacing sha384 {sha384} with {str(value)}")
...
class Publish(object):
    ...
    def beginPublishing(self):
        self.logger.statusMessage(f"Beginning publish of warehouse: '{self.wh_TLC}' at location: '{self.wh_path}'")
        ...
        for i in range(0, 3):
            ...
            if not os.path.exists(warehouse_dir):
                self.logger.statusMessage(f"Directory: '{warehouse_dir}' does not exist - creating")
                try:
                    os.makedirs(warehouse_dir)
                except OSError as Error:
                    self.logger.errorMessage("Failed to create directory", Error)
            else:
                self.logger.statusMessage(f"Directory: '{warehouse_dir}' already exists")
        ...
        for child_object in self.li_child_objects:
            self.logger.statusMessage(f"Publishing package: '{child_object.en_long_desc}'")
            ...
            self.logger.statusMessage(f"Creating 'badger:attributes' XML document at: '{self.wh_path}'")
        ...
        for product_instance_object in self.li_product_instances:
            ...
            self.logger.statusMessage(f"Publishing package for product instance: '{product_instance_object.parent_object.en_long_desc}'")
            ...
            for child_object_ID in product_instance_object.child_object_IDs:
                ...
                if matched_child_object:
                    ...
                    if decode_path is None:
                        ...
                        self.logger.errorMessage(f"Using subpackage default home folder: {decode_path}")

                    self.logger.statusMessage(f"Adding package: '{matched_child_object.en_long_desc}' to subpackage list decode_path={decode_path}")
                    ...
                    self.logger.errorMessage(f"Could not find already published package against product instance's ID: '{child_object_ID}'")
                    self.error_state = 1
        ...
        for k, v in dic_rigid_object.items():
            v.save(k)
        ...
        self.logger.statusMessage(f"Creating 'badger:ultimate' XML document at: '{self.wh_path}'")
        ...
        self.logger.statusMessage(f"Creating 'badger:catalogue' XML document at: '{self.wh_path}'")
    ...
    def addRigidobjectToDictionary(self, dic_rigid_object, package_object):
        ...
        if package_object.rigid_name not in dic_rigid_object:
            self.logger.statusMessage(f"Creating 'badger:rigid' XML document for: '{package_object.rigid_name}' at: '{self.wh_path}'")
            ...
            if not os.path.exists(GUID_dir):
                self.logger.statusMessage(f"Directory: '{GUID_dir}' does not exist - creating")
                try:
                    os.makedirs(GUID_dir)
                except OSError as Error:
                    self.logger.errorMessage("Failed to create directory", Error)
            else:
                self.logger.statusMessage(f"Directory: '{GUID_dir}' already exists")
        ...
        self.logger.statusMessage(f"Adding version: '{package_object.version}' to 'badger:rigid' XML document for: '{package_object.rigid_name}'")
    ...
    def matchPackageObjectToID(self, ID_to_find):
        for child_object in self.li_child_objects:
            if child_object.object_ID == ID_to_find:
                self.logger.statusMessage(f"Matched object ID: '{ID_to_find}' to package: '{child_object.en_long_desc}'")
                return child_object

        return None

    def signFile(self, file_name, logger):
        self.logger.statusMessage(f"Signing: '{file_name}' with: '{sdds_platform.MANIFEST_DAT}'")
        self.logger.statusMessage(f"Using private key: '{sdds_platform.PRIVATEKEY_PEM}'")
        self.logger.statusMessage(f"Using public certificate: '{sdds_platform.PUBLICCERT_CRT}'")
        self.logger.statusMessage(f"Using intermediate certificate: '{sdds_platform.INTERCERT_CRT}'")
        self.logger.statusMessage(f"Using OpenSSL at location: '{sdds_platform.OPENSSL}'")
        self.logger.statusMessage(f"Using manifest.dat at location: '{sdds_platform.MANIFEST_DAT}'")
    ...
    def copyFiles(self, import_path, li_file_data):
        ...
        assert os.path.exists(package_path), f"{package_path} doesn't exist - can't possibly import from it"
        ...
        for file_data in li_file_data:
            ...
            if os.path.exists(current_file_path):
                ...
                if file_MD5 != file_data.MD5:
                    self.logger.errorMessage(f"Checksum of file: '{current_file_path}' differs to that in SDDS-import file")
                    self.error_state = 1
                else:
                ...
                if int(file_size) != int(file_data.size):
                    self.logger.errorMessage(f"Size of file: '{current_file_path}' differs to that in SDDS-import file")
                    self.error_state = 1
                ...
                if self.error_state == 0:
                    ...
                    self.logger.statusMessage(f"Copying source file: '{current_file_path}' to target: '{target_path}' ")
                ...
                if self.error_state == 0:
                    ...
                    except IOError as error_pair:
                        (errno, strerror) = error_pair.args
                ...
                self.logger.errorMessage(f"File: '{current_file_path}' doesn't exist in source")
...
class PackageInstanceContents(object):
    ...
    def __init__(self, package_object, wh_path, logger):
        ...
        logger.statusMessage(f"Creating 'badger:contents' XML document for: '{package_object.dictionary_label_token}' at: '{wh_path}'")
        ...
        key = f"badger_package_contents_{PackageInstanceContents.BADGER_PACKAGE_CONTENTS_COUNT}"
        PackageInstanceContents.BADGER_PACKAGE_CONTENTS_COUNT += 1
        global GL_DELIBERATE_ERRORS
        GL_DELIBERATE_ERRORS.add_SHA384_errors(self.contents_object, key)

        logger.statusMessage(f"Creating 'badger:package' XML document for: '{package_object.dictionary_label_token}' at: '{wh_path}'")
...
class XMLPrototype(object):
    ...
    def saveXMLDocument(self, wh_path, dir=""):
        XML_path = os.path.join(wh_path, f"{dir}.xml")

        self.logger.statusMessage(f"Creating XML meta data file: '{XML_path}'")
    ...
    def saveXMLDocumentAsMD5(self, wh_path, dir=""):
        ...
        if not os.path.exists(temp_XML_path):
            ...
            raise Exception(f"Failed to create temporary file {temp_XML_path}")
        ...
        self.logger.statusMessage(f"Creating XML meta data file: '{MD5_XML_path}'")
...
class MetaDataProductLineRigid(XMLPrototype):
    ...
    def addProductInstance(self, package_object):
        ...
        assert (isinstance(package_object.packageMetaSha384(), str)), f"Wrong type for packageMetaSha384: {type(package_object.packageMetaSha384())} - " \
                                                                      f"{str(package_object.packageMetaSha384())} - " \
                                                                      f"{repr(package_object.packageMetaSha384())}"
        assert (isinstance(package_object.packageMetaSize(), str))
...
class MetaDataUltimate(XMLPrototype):
    ...
    def getPath(self):
        return os.path.join(self.wh_path, f"{self.getMD5() + 'x000'}.xml")
    ...
    def __init__(self, package_object, wh_path, logger, subcomponent=False):
        ...
        if package_object.release_tags:
            ...
            for release_tag in package_object.release_tags:
                ...
                if release_tag.tag in ReleaseTagEnumerations.release_tag_enums:
                    self.XML_object.chlTxtNode("node4", ReleaseTagEnumerations.release_tag_enums[release_tag.tag][base])
                else:
                    logger.errorMessage(f"Release tag: '{release_tag.tag}' is invalid")
        else:
            if not subcomponent:
