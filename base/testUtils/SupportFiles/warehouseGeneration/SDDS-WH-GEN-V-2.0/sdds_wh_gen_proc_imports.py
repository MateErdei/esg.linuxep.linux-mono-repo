...
class ProcessSDDSImports(object):
    ...
    def _load(self, comp_import_path):
        ...
        self.logger.statusMessage(f"Opening SDDS import file from: {comp_import_path}")
        ...
        self.logger.statusMessage(f"Parsing data from SDDS import file from: {comp_import_path}")
        ...
        try:
            ...
            print(f"Failed to read {comp_import_path}", file=sys.stderr)
            data = open(comp_import_path, "rb").read()
            print(f"File contents: ###{data}###", file=sys.stderr)
            raise

        self.logger.statusMessage(f"Sucessfully opened and parsed SDDS import file from: {comp_import_path}")
    ...
    def processImports(self):
        ...
        for product_instance in self.li_product_instances:
            ...
            if product_instance.parent_object is not None:
                self.logger.statusMessage(f"Not processing duplicate instance with component suite import file: '{comp_import_path}'")
                continue

            assert product_instance.parent_object is None
            self.logger.statusMessage(f"Processing product instance with component suite import file: '{comp_import_path}'")
            ...
            for child_component_data in product_instance.child_component_data:
                ...
                if create_new_package:
                    self.logger.statusMessage(f"New SDDS-import file found. Creating object ID: '{str(packageID)}'")
                    ...
                    if child_object.product_type != "Component":
                        ...
                        self.logger.statusMessage(f"Linking product instance: '{product_instance.parent_object.en_long_desc}' to package ID: '{str(packageID)}'")
                    ...
                    self.logger.statusMessage(f"Package already exists. Ignoring and using package ID: '{str(existing_child_package_ID)}'")
                    self.logger.statusMessage(f"Linking product instance: '{product_instance.parent_object.en_long_desc}' to package ID: '{str(existing_child_package_ID)}'")
        ...
        for pi in self.li_product_instances:
            ...
            self.logger.statusMessage(f"*** Got Product instance with rigid_name={parent_object.rigid_name}, version={parent_object.version}, parent_object={str(parent_object)}")
    ...
    def extractDataFromImport(self, XML_document, instance_object):
        ...
        if component_nodes.length == 1:
            ...
            if not object_incomplete:
                ...
                self.logger.statusMessage(f"Name: '{instance_object.name}'")
                self.logger.statusMessage(f"Rigid Name: '{instance_object.rigid_name}'")
                self.logger.statusMessage(f"Version: '{instance_object.version}'")
                self.logger.statusMessage(f"Build: '{instance_object.build}'")
                self.logger.statusMessage(f"Product Type: '{instance_object.product_type}'")
                self.logger.statusMessage(f"Default Home Folder: '{instance_object.default_home_folder}'")
                    ...
                    if component_child_node.tagName == "TargetTypes":
                        ...
                        for k in range(0, len(component_child_node.childNodes)):
                            if component_child_node.childNodes[k].nodeType == 1:
                                ...
                                self.logger.statusMessage(f"Adding target type '{target_type}'")
                        ...
                        for k in range(0, len(component_child_node.childNodes)):
                            if component_child_node.childNodes[k].nodeType == 1:
                                ...
                                self.logger.statusMessage(f"Adding role '{role}'")
                        ...
                        for k in range(0, len(component_child_node.childNodes)):
                            if component_child_node.childNodes[k].nodeType == 1:
                                ...
                                self.logger.statusMessage(f"Adding platform '{platform}'")
                        ...
                        for k in range(0, len(component_child_node.childNodes)):
                            if component_child_node.childNodes[k].nodeType == 1:
                                ...
                                self.logger.statusMessage(f"Adding platform '{feature}'")
                        ...
                        self.logger.statusMessage(f"SAV Line: '{instance_object.SAV_line}'")
                        ...
                        self.logger.statusMessage(f"SAV Version: '{instance_object.SAV_version}'")
                        ...
                        self.logger.statusMessage(f"Virus Data Version: '{instance_object.virus_data_version}'")
                        ...
                        self.logger.statusMessage(f"Virus Engine Version: '{instance_object.virus_engine_version}'")
                        ...
                        for filenode in component_child_node.childNodes:
                            ...
                            if "Offset" in list(filenode.attributes.keys()):
                            ...
                            self.logger.statusMessage(f"Adding file '{file_data.name}' MD5={file_data.MD5}")
            ...
            for i in range(0, len(label_nodes[0].childNodes)):
                if label_nodes[0].childNodes[i].nodeType == 1:
                    ...
                    if label_child_node.tagName == "Token":
                        ...
                        self.logger.statusMessage(f"Token: '{instance_object.dictionary_label_token}'")
                    ...
                    if label_child_node.tagName == "Language":
                        ...
                        self.logger.statusMessage(f"Short Description: '{instance_object.en_short_desc}'")
                        instance_object.en_long_desc = label_child_node.childNodes[1].firstChild.data
                        self.logger.statusMessage(f"Long Description: '{instance_object.en_long_desc}'")
