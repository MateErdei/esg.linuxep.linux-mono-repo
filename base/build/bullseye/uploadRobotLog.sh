...
DEST_DIR=public_html/robot_base/$(dirname $DEST_FILE)
ssh -i ${PRIVATE_KEY} -o StrictHostKeyChecking=no "upload@allegro.eng.sophos" mkdir -p "$DEST_DIR" </dev/null
rsync -az --rsh="ssh -i ${PRIVATE_KEY} -o StrictHostKeyChecking=no" \
    "$SRC_FILE" \
    "upload@allegro.eng.sophos:public_html/robot_base/$DEST_FILE"  \
    </dev/null \
