                        ...
                        try:
                            if error['inconclusive'] == 'true':
                                ...
                                if error.get('verbose') and (error['verbose'] != error['msg']):
                                    index = t.rfind('\n')
                                    t = t[:index] + HTML_EXPANDABLE_INCONCLUSIVE % (
                                    error['msg'], html_escape(error['verbose'].replace("\\012", '\n'))) + t[index + 1:]
                                else:
                                    t = t.replace('\n', HTML_INCONCLUSIVE % error['msg'])
                        except KeyError:
                            if error.get('verbose') and (error['verbose'] != error['msg']):
                                index = t.rfind('\n')
                                t = t[:index] + HTML_EXPANDABLE_ERROR % (
                                error['msg'], html_escape(error['verbose'].replace("\\012", '\n'))) + t[index + 1:]
                            else:
...
if __name__ == '__main__':
    ...
    for filename, data in sorted(files.items()):
        ...
        with io.open(os.path.join(options.report_dir, htmlfile), 'w', encoding='utf-8') as output_file:
            ...
            for error in sorted(errors, key=lambda k: k['line']):
                output_file.write(
                    "<a href='%s#line-%d'> %s %s</a>" % (data['htmlfile'], error['line'], error['id'], error['line']))
            ...
            try:
                ...
                sys.stderr.write(
                    "ERROR: Couldn't determine lexer for the file' " + source_filename + " '. Won't be able to syntax highlight this file.")
                output_file.write(
    ...
    with io.open(os.path.join(options.report_dir, 'index.html'),
        ...
        output_file.write(
            HTML_HEAD.replace('id="menu" dir="rtl"', 'id="menu_index"', 1).replace("Defects:", "Defect summary;", 1) % (
        ...
        for filename, data in sorted(files.items()):
            if filename in decode_errors:  # don't print a link but a note
                ...
                for error in sorted(data['errors'], key=lambda k: k['line']):
                    ...
                    try:
                        if error['cwe']:
                            cwe_url = "<a href='https://cwe.mitre.org/data/definitions/" + error['cwe'] + ".html'>" + \
                                      error['cwe'] + "</a>"
                    except KeyError:
                        cwe_url = ""
    ...
    with io.open(os.path.join(options.report_dir, 'stats.html'), 'w') as stats_file:
        ...
        stats_file.write(
            HTML_HEAD.replace('id="menu" dir="rtl"', 'id="menu_index"', 1).replace("Defects:", "Back to summary", 1) % (
            options.title, '', options.title, 'Statistics', ''))
        ...
        for sev in SEVERITIES:
            ...
            for i in stats_list_sorted:  # printing loop
                ...
                if (it == 0):
                    LENGTH = len(str(i[
                                         1]))  # <- length of longest number, now get the difference and try to  make other numbers align to it

                stats_file.write(
                    "&#160;" * 3 + str(i[1]) + "&#160;" * (1 + LENGTH - len(str(i[1]))) + "<a href=\"" + files[i[0]][
                        'htmlfile'] + "\">  " + i[0] + "</a></br>\n")
                it += 1
