...
TEST_F(PluginAdapterTests, emptyActionTypeIsThrownAway)
{
    ...
    m_taskQueue->push(
        ResponsePlugin::Task{ ResponsePlugin::Task::TaskType::ACTION, "{\"type\":\"\",\"timeout\":1000}" });
    m_taskQueue->pushStop();
    UsingMemoryAppender recorder(*this);
    pluginAdapter.mainLoop();

    EXPECT_TRUE(appenderContains("Throwing away unknown action: {\"type\":\"\",\"timeout\":1000}"));
}
...
TEST_F(PluginAdapterTests, ActionWithMissingTimeoutIsThrownAway)
{
    ...
    m_taskQueue->push(ResponsePlugin::Task{ ResponsePlugin::Task::TaskType::ACTION, "{\"type\":\"something\"}" });
    m_taskQueue->pushStop();
    UsingMemoryAppender recorder(*this);
    pluginAdapter.mainLoop();
...
TEST_F(PluginAdapterTests, UploadFileActionTriggersRun)
{
    ...
    m_taskQueue->push(ResponsePlugin::Task{ ResponsePlugin::Task::TaskType::ACTION,
                                            "{\"type\":\"sophos.mgt.action.UploadFile\",\"timeout\":1000}" });
    m_taskQueue->pushStop();
    UsingMemoryAppender recorder(*this);
...
TEST_F(PluginAdapterTests, TestActionsRunInOrder)
{
    ...
    m_taskQueue->push(ResponsePlugin::Task{
        ResponsePlugin::Task::TaskType::ACTION, "{\"type\":\"a\",\"timeout\":1}", "CORE", "id1" });
    m_taskQueue->push(ResponsePlugin::Task{
        ResponsePlugin::Task::TaskType::ACTION, "{\"type\":\"a\",\"timeout\":1}", "CORE", "id2" });
    m_taskQueue->push(ResponsePlugin::Task{
        ResponsePlugin::Task::TaskType::ACTION, "{\"type\":\"a\",\"timeout\":1}", "CORE", "id3" });
    m_taskQueue->pushStop();
