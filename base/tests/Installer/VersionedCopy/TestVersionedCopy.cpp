...
namespace
{
    ...
    int versionedCopyInTempDir(const Tests::TempDir& tempdir, const Path& filename, const Path& dist, const Path& inst)
    {
        return VersionedCopy::versionedCopy(tempdir.absPath(filename), tempdir.absPath(dist), tempdir.absPath(inst));
    }

    int symlinkInTempDir(const Tests::TempDir& tempdir, const Path& target, const Path& dest)
    {
    ...
    bool sameInTempDir(const Tests::TempDir& tempdir, const Path& file1, const Path& file2)
    {
