...
class TestMCS(unittest.TestCase):
    ...
    @mock.patch('os.listdir', return_value=[])
    @mock.patch('mcsrouter.adapters.app_proxy_adapter.AppProxyAdapter.get_migrate_action', return_value="""<?xml version='1.0'?><action type="sophos.mgt.mcs.migrate"><server>http://es-web.sophos.com/update</server><token>jwt</token></action>""")
    ...
    def testMigrationAction(self, *mockargs):
        ...
        self.assertEqual(root_config.get('MCSURL'), 'http://es-web.sophos.com/update')
