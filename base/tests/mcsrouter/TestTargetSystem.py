...
class TestTargetSystem(unittest.TestCase):
    ...
    # OS Version Tests
    @mock.patch('mcsrouter.targetsystem.TargetSystem._collect_lsb_release', return_value=UBUNTU_3_VERSION)
    ...
    @mock.patch('mcsrouter.targetsystem.TargetSystem._collect_lsb_release', return_value=UBUNTU_2_VERSION)
    def test_os_version_retrieves_two_part_ubuntu_version(self, *mockargs):
        target_system = mcsrouter.targetsystem.TargetSystem('/tmp/sophos-spl')
        self.assertEqual(('18', '04', ''), target_system.os_version())

    @mock.patch('mcsrouter.targetsystem.TargetSystem._collect_lsb_release', return_value=NO_LSB)
    def test_os_version_retrieves_two_part_ubuntu_version(self, *mockargs):
        target_system = mcsrouter.targetsystem.TargetSystem('/tmp/sophos-spl')
    ...
    @mock.patch('mcsrouter.targetsystem.TargetSystem._collect_lsb_release', return_value=CENTOS_LSB_1)
    def test_os_version_retrieves_centos_major_version(self, *mockargs):
        target_system = mcsrouter.targetsystem.TargetSystem('/tmp/sophos-spl')
        self.assertEqual('centos', target_system.vendor())
        self.assertEqual(['7', '', ''], target_system.os_version())

    @mock.patch('mcsrouter.targetsystem.TargetSystem._collect_lsb_release', return_value=CENTOS_LSB_2)
    def test_os_version_retrieves_centos_major_and_minor_version(self, *mockargs):
        target_system = mcsrouter.targetsystem.TargetSystem('/tmp/sophos-spl')
        self.assertEqual('centos', target_system.vendor())
        self.assertEqual(['7', '4', ''], target_system.os_version())

    @mock.patch('mcsrouter.targetsystem.TargetSystem._collect_lsb_release', return_value=CENTOS_LSB_3)
    ...
    def test_read_uname_calls_os_uname(self, *mockargs):
        target_system = mcsrouter.targetsystem.TargetSystem('/tmp/sophos-spl')
        uname = target_system._read_uname()
