...
TEST_F(ActionTaskTests, ActionTaskHandlesNameWithSingleUnderscore) // NOLINT
{
    ...
    ManagementAgent::McsRouterPluginCommunicationImpl::ActionTask task(
            m_mockPluginManager, "/tmp/action/_");
    task.run();
}
...
TEST_F(ActionTaskTests, TooLongAndTooShortLiveResponseFilenamesNotProcessedAsGenericActions)
{
    ...
    std::string file_path1 = "/tmp/action/LiveTerminal_correlation-id_action_123456.xml";
    std::string file_path2 = "/tmp/action/LiveTerminal_extra-argument_correlation-id_action_FakeTime_123456.xml";
...
TEST_F(ActionTaskTests, TooLongAndTooShortLiveQueryFilenamesNotProcessedAsGenericActions)
{
    ...
    auto filesystemMock = new StrictMock<MockFileSystem>();
    EXPECT_CALL(*filesystemMock, readFile(_)).Times(0);
    std::string file_path1 = "/tmp/action/LiveQuery_correlation-id_request_123456.xml";
    std::string file_path2 = "/tmp/action/LiveQuery_extra-argument_correlation-id_request_FakeTime_123456.xml";
    EXPECT_CALL(*filesystemMock, removeFile(file_path1)).Times(0);
    EXPECT_CALL(*filesystemMock, removeFile(file_path2)).Times(0);
    Tests::replaceFileSystem(std::unique_ptr<Common::FileSystem::IFileSystem>(filesystemMock));
