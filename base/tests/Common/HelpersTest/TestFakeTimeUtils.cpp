...
{
    bool stop{false};
    SequenceOfFakeTime sequenceOfFakeTime({t_20190501T13h}, std::chrono::milliseconds(10),[&stop](){stop=true;});
...
TEST(TestFakeTimeUtils, SequenceOfFakeTimeNEntriesAppliesTheCacheTime) // NOLINT
{
    ...
    while( !stop)
    {
        ...
        auto current_time = sequenceOfFakeTime.getCurrentTime();
        // it does not need to check it the value changed as the amount of time 'sleeping' is bigger than the 'cacheWindowTime'
        newValues.push_back(current_time);
...
TEST(TestFakeTimeUtils, DemonstrationHowTheFakeTimeIsToBeHookedIntoTimeUtils) // NOLINT
{
    ...
    Common::UtilityImpl::ScopedReplaceITime scopedReplaceITime(
            std::unique_ptr<Common::UtilityImpl::ITime>(
