...
namespace
{
    ...
    public:
        ProcessMonitorThread(Common::ProcessMonitoring::IProcessMonitorPtr processMonitorPtr)
                : m_processMonitorPtr(std::move(processMonitorPtr)), m_thread_id{}
        {}
...
TEST_F(ProcessMonitoring, createProcessMonitorSocketHandlersThatThrowExceptionsThatAreCaught)  //NOLINT
{
    ...
    processMonitorPtr->addReplierSocketAndHandleToPoll(replierSocket1.get(),
            [replierSocket1BarePtr]() {
    ...
    processMonitorPtr->addReplierSocketAndHandleToPoll(replierSocket2.get(),
            [replierSocket2BarePtr]() {
    ...
    processMonitorPtr->addReplierSocketAndHandleToPoll(blockingReplierSocket.get(),
                                                       [blockingReplierSocketBarePtr]() {
    ...
    std::string logMessage = testing::internal::GetCapturedStderr();
    EXPECT_THAT(logMessage, ::testing::Not(::testing::HasSubstr("No processes to monitor!")));
    EXPECT_THAT(logMessage, ::testing::HasSubstr("Unexpected error occurred when handling socket communication: IPC error"));
    ...
    EXPECT_THAT(logMessage, ::testing::Not(::testing::HasSubstr("Unexpected error occurred when handling socket communication: IPC error")));
