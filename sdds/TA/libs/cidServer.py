...
class DigestAuthentication(Authentication):
    ...
    def authenticate(self, handler):
        (code, headers, username) = self.__authenticate(handler.command,
                                                        handler.path,
...
class PasswordHTTPRequestHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if PasswordHTTPRequestHandler.authHandler is not None:
...
def parseOptions(args):
    ...
    addOptions(parser)
    return getOptionsFromArgs(parser, args)
...
def create_server(options):
    ...
    handler_class = functools.partial(handler_class, directory=options.root)
...
class CidServerThread(threading.Thread):
