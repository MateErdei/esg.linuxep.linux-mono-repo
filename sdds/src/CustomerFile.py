...
import locations
from warehouse_sign.sign import get_certificate, pem_signature
...
def sign(dci_file_path):
    ...
    """
    with open(dci_file_path, "a+b") as dci_file:
        try:
            dci_file.write(pem_signature(dci_file_path))
        except Exception as e:
            raise Exception("ERROR - Signing Failed: %s" % e)

        for cert in ("pub", "ca"):
            try:
                dci_file.write(get_certificate(cert))
            except Exception as e:
                raise Exception("ERROR - Failed to get cert %s: %s" % (cert, e))


def create(warehouse_def_file):
    """
    Create customer files according to the specified list in yaml format.
    """
    wh_input = open(warehouse_def_file).read()
    ...
    for c in customer_files:
        customer_file_name = hashlib.md5(f'{c["user"]}:{c["password"]}'.encode()).hexdigest()
        file_name_element_value = hashlib.md5(customer_file_name.encode()).hexdigest()
        if 'output-dir' in c:
        ...
        urls = ""
        for url in locations.WAREHOUSE_READ:
        ...
        if split_whs != "":
            ...
            content = NEW_DCI_MAGIC_TEXT.replace('\r\n', '\n') % (
                repo_id, current_time_str, current_time_str, file_name_element_value,
                urls, wh_block, redirects_block)
            with open(dci_file_path, "w+b") as ef:
                ef.write(content.encode())
            sign(dci_file_path)
