...
hex_to_int_response = """{
        "type": "sophos.mgt.response.RunLiveQuery",
        "queryMetaData": {"durationMillis":1,"errorCode":0,"errorMessage":"OK","rows":1,"sizeBytes":14},
        "columnMetaData": [{"name":"hex_string","type":"TEXT"},{"name":"int","type":"TEXT"}],
        "columnData":  [["0xf","15"]]
    }"""
...
grep_query = """{
    "type": "sophos.mgt.action.RunLiveQuery",
    "name": "grep file",
    "query": "SELECT pattern FROM grep WHERE path = '/etc/fstab' AND pattern = 'UUID'"
}"""
...
def test_edr_plugin_expected_responses_to_livequery(sspl_mock, edr_plugin_instance):
    ...
    edr_plugin_instance.wait_for_osquery_to_be_up_and_stable()
