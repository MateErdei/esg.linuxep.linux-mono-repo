...
class Plugin(object):
    ...
    def action(self, app_id, action):
        self.logger.info(b"Sending %s action to %s via %s" % (action, self.name, self.__m_socket_path))
    ...
    def get_status(self, app_id):
        self.logger.info(b"Request Status for " + self.name)
        ...
        if response.acknowledge or response.error or (response.contents and len(response.contents) < 2):
            self.logger.error(b"No status in message from plugin: {}, contents: {}".format(response.plugin_name, response.contents))
            return ""
        self.logger.info(b"Got status back from plugin, plugin name: {}, status: {}".format(self.name, str(response.contents)))
    ...
    def get_telemetry(self):
        self.logger.info(b"Request Telemetry for " + self.name)
        ...
        if response.acknowledge or response.error or (response.contents and len(response.contents) < 1):
            self.logger.error(b"No telemetry in message from plugin: {}, contents: {}".format(response.plugin_name, response.contents))
            return ""
        self.logger.info(b"Got telemetry back from plugin, plugin name: {}, status: {}".format(self.name, str(response.contents)))
...
# Controller message handlers and related functions
    ...
    def handle_control_queue_reply(self, control_message):
        ...
        # TODO LINUXEP-6547: Deal with invalid messages, allow them to be sent even if they are not valid actions.
