...
public:
    ...
    TestOSQueryProcessorWithMock() : m_mockClientPtr(new MockIOsqueryClient()), m_testFakeSocketPath(("/fake/osquery.sock"))
    {
    }
...
TEST_F(TestOSQueryProcessorWithMock, SimpleSelectShouldReturn) // NOLINT
{
    ...
    EXPECT_CALL(*mockClient, query(sql, _)).WillOnce(DoAll(::testing::SetArgReferee<1>(queryData), Return(osquery::Status::success())));
    EXPECT_CALL(*mockClient, getQueryColumns(sql, _)).WillOnce(DoAll(::testing::SetArgReferee<1>(queryColumnHeaders), Return(osquery::Status::success())));
...
TEST_F(TestOSQueryProcessorWithMock, NoOSqueryAvailableShouldReturnError) // NOLINT
{
    ...
    EXPECT_CALL(*mockClient, connect(_)).WillOnce(Throw(osqueryclient::FailedToStablishConnectionException("\"Could not connect to osquery after 1 second\"")));
...
TEST_F(TestOSQueryProcessorWithMock, UnsuccessfulExecutionsStatusFromGetQueryColumnsWillReturnError) // NOLINT
{
    ...
    EXPECT_CALL(*mockClient, connect(m_testFakeSocketPath));
    EXPECT_CALL(*mockClient, query(_, _)).WillOnce(DoAll(::testing::SetArgReferee<1>(queryData), Return(osquery::Status::success())));
    EXPECT_CALL(*mockClient, getQueryColumns(_, queryData)).WillOnce(Return(osquery::Status::failure("error retrieving query column details")));
...
TEST_F(TestOSQueryProcessorWithMock, OsQueryClientExceptionsFromGetQueryColumnsAreHandled) // NOLINT
{
    ...
    EXPECT_CALL(*mockClient, connect(m_testFakeSocketPath));
    EXPECT_CALL(*mockClient, query(sql, _)).WillOnce(DoAll(::testing::SetArgReferee<1>(queryData), Return(osquery::Status::success())));
    EXPECT_CALL(*mockClient, getQueryColumns(sql, queryData)).WillOnce(Throw(apache::thrift::transport::TTransportException()));
...
TEST_F(TestOSQueryProcessorWithMock, HeaderWillBeDeducedIfNotCompletellyGivenByOsQueryAndTheTypeWillAlwaysBeText) // NOLINT
{
    setupMockClient();
    auto& mockClient = m_mockClientPtr;
