...
//    }
    std::unique_ptr<livequery::IQueryProcessor> queryProcessor(new osqueryclient::OsqueryProcessor{});
    std::unique_ptr<livequery::IResponseDispatcher> queryResponder(new livequery::ResponseDispatcher{});

    auto response = queryProcessor->query("select * from processes");
    queryResponder->sendResponse("thisid", response);
//
//
