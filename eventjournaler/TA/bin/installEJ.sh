...
INPUTS_ROOT=$BASE/../..
EJ_ROOT=$INPUTS_ROOT
[[ -d $EJ_ROOT/event_journaler_sdds ]] || EJ_ROOT=/opt/test/inputs
EJ_SDDS=$EJ_ROOT/event_journaler_sdds
[[ -f "${EJ_SDDS}/manifest.dat" ]] || failure 1 "Can't find ${EJ_SDDS}/manifest.dat"
TEST_SUITE=${BASE}/..
...
chmod 700 "${EJ_SDDS}/install.sh"
bash $INSTALL_EJ_BASH_OPTS "${EJ_SDDS}/install.sh" || failure 6 "Unable to install SPL-EJ: $?"
