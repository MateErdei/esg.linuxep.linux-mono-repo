...
SYSTEM_TEST_BULLSEYE_JENKINS_JOB_URL = 'https://sspljenkins.eng.sophos/job/SSPL-Plugin-Device-Isolation-bullseye-system-test-coverage/build?token=sspl-linuxdarwin-coverage-token'
# For branch names, remember that slashes are replaced with hyphens:  '/' -> '-'
SYSTEM_TEST_BULLSEYE_CI_BUILD_BRANCH = 'develop'

COVFILE_UNITTEST = '/opt/test/inputs/coverage/sspl-plugin-deviceisolation-unit.cov'
COVFILE_TAPTESTS = '/opt/test/inputs/coverage/sspl-plugin-deviceisolation-tap.cov'
...
def get_inputs(context: tap.PipelineContext, di_build: ArtisanInput, di_x86_64_build: ArtisanInput, mode: str, arch: str):
    if di_build is None:
        return None
    if di_x86_64_build is None:
        return None
    ...
    if mode == 'release':
        ...
        test_inputs = dict(
            ...
            device_isolation_sdds=di_build / f'deviceisolation/{config}/installer',
            base_sdds=di_build / f'base/{config}/installer',
            fake_management=di_x86_64_build / 'base/fake_management',
        )
    if mode == 'coverage':
        test_inputs = dict(  # TODO check this by setting mode and tap ls
            ...
            SupportFiles=context.artifact.from_folder('./base/testUtils/SupportFiles'),
            tests=context.artifact.from_folder('./base/testUtils/tests'),
            device_isolation_sdds=di_build / 'sspl-plugin-deviceisolation-coverage/SDDS-COMPONENT',
            bullseye_files=context.artifact.from_folder('./deviceisolation/build/bullseye'),
            coverage=di_build / 'sspl-plugin-deviceisolation-coverage/covfile',
            coverage_unittest=di_build / 'sspl-plugin-deviceisolation-coverage/unittest-htmlreport',
            base_sdds=di_build / 'sspl-plugin-deviceisolation-coverage/base/base-sdds',
            fake_management=di_x86_64_build / 'sspl-plugin-deviceisolation-coverage/base/fake-management',
...
def coverage_task(machine: tap.Machine, branch: str, robot_args: str):
    try:
        ...
        unitest_htmldir = os.path.join(INPUTS_DIR, "sspl-plugin-deviceisolation-unittest")
        ...
        tap_htmldir = os.path.join(INPUTS_DIR, 'sspl-plugin-deviceisolation-taptest')
...
def run_di_coverage_tests(stage, context, di_coverage_build, mode, parameters):
    # # Modes where we do not want to run TAP tests.
    # if mode in [ANALYSIS_MODE]:
    #     return

    test_inputs = get_inputs(context, di_coverage_build, mode, "x64")
    robot_args = get_robot_args(parameters)

    with stage.parallel('di_coverage'):
...
def run_di_tests(stage, context, builds, mode, parameters):
    # # Modes where we do not want to run TAP tests.
    # if mode in [ANALYSIS_MODE]:
    #     return
    ...
    with stage.parallel('deviceisolation_integration'):
