...
#include <modules/sessionrunner/ParallelSessionProcessor.h>
...
public:
    ...
    void triggerSession(const std::string& id, std::function<void(std::string id)> notifyFinished) override
    {
        *m_counter+=1;
        triggered = true;
        m_id = id;
        m_fut = std::async(std::launch::async, [id, notifyFinished](){
...
TEST_F(ParallelSessionProcessorTests, tenJobsAreAddedAndClearedSuccessfully) // NOLINT
{
    ...
    {
        sessionrunner::ParallelSessionProcessor parallelQueryProcessor{std::unique_ptr<sessionrunner::ISessionRunner>(new ConfigurableDelayedSession{counter}), "/somepath"};
        for(int i=0; i<10;i++)
        {
