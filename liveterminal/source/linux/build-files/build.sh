...
VERSION=$(grep "<package name=\"liveterminal_linux\"" $BASE/linux-release-package.xml | sed -e s/.*version=\"// -e s/\"\>//  -e s/-/\./g )
echo "version = '${VERSION}'" >> ${SRCROOT}/CIJenkinsfile
...
BULLSEYE_UPLOAD=0
ANALYSIS=0
COVFILE="${BASE}/liveterminal_unittests.cov"
...
function build()
{
    ...
    then
        ...
        export BULLSEYE_DIR
        bash -x "${BUILD_FILES}/createCovFile.sh" "${BASE}" || exitFailure $FAILURE_BULLSEYE_FAILED_TO_CREATE_COVFILE "Failed to create covfile: $?"
