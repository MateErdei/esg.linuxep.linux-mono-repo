...
namespace sessionrunner
{
    ...
    {
        m_id = id;
        m_fut = std::async(std::launch::async, [this, id, notifyFinished]() {
            try
            {
    ...
    std::string SessionRunnerImpl::id() { return m_id; }
    void SessionRunnerImpl::setStatusFromExitResult(const std::string& id, SessionRunnerStatus& sessionRunnerStatus, int exitCode, const std::string& output)
    {
        if (exitCode != 0)
        {
            ...
            if (output.empty())
            {
                LOGERROR("Execution of live response session id '" << id << "' failed with error code: " << exitCode << ", " << strsignal(exitCode));
            }
            else
            {
                LOGERROR("Execution of live response session id '" << id << "' failed with error code: " << exitCode << ", " << strsignal(exitCode) << " and output: " << output);
            }
    ...
    void SessionRunnerImpl::requestAbort() { m_process->kill(); };
